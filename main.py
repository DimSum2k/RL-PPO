...
if __name__ == '__main__':
	...
	for loss in ["clipped_loss","adaptative_KL_loss","A2C_loss"]:
		...
		# save models
		torch.save(agent.actor_network.state_dict(), os.path.join(folder_name,"weights",loss +"_actor.pth"))
		torch.save(agent.value_network.state_dict(), os.path.join(folder_name,"weights",loss +"_critic.pth"))
	...
	f.close()
	pkl.dump({"config":config,"loss":loss_list,"rewards":rewards_list},open(os.path.join(folder_name,"logs","losses_rewards_config.pkl"),"wb"))