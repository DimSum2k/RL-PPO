...
def get_arguments():
    ...
    parser.add_argument('--eps_clipping',type=float, help='clipping parameter for ppo clipped loss (range : 0.1-0.3)', default=0.2)
    ...
    parser.add_argument('--c1',type=float, help='value function weight if shared actor-critic architecture', default=0)
    ...
    parser.add_argument('--max_steps', type=int, help='maximum number of steps per episode', default=300) 
    parser.add_argument('--optimize_every', type=int, help='size of trajectories we learn on', default=2048)
    parser.add_argument('--batch_size', type=int, default=2048)
    parser.add_argument('--epochs', type=int, default=4)
    parser.add_argument('--reward_norm', action='store_true', help='if true normalise rewards', default=0)
...
def reset_config(opt, print_=False):
    ...
    config = {}
    assert opt.env in ['CartPole-v1','MountainCarContinuous-v0','LunarLander-v2','MountainCar-v0']
    config['env'] = opt.env
    config['std'] = opt.std
